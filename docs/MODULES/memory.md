...
- `embedded/embedded-manager.ts` - embedded embeddings + hybrid vector/FTS backend
...
### Embedded Memory Backend (`memory.embedded`)

Embedded memory uses local SQLite + embeddings for vector search with optional FTS hybrid search. It supports OpenAI-compatible embedding endpoints (including local Ollama) and keeps Mozi paths (`MEMORY.md` and `memory/*.md`).
...
To include session transcripts in embedded search, set:
