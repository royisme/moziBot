...
async function getRelayAuthToken() {
  const stored = await chrome.storage.local.get(['relayAuthToken'])
...
async function ensureRelayConnection() {
  ...
  relayConnectPromise = (async () => {
    ...
    const relayAuthToken = await getRelayAuthToken()
    const httpBase = `http://127.0.0.1:${port}`
    const wsUrl = await buildRelayWsUrl(port, relayAuthToken)
  ...
  for (const [tabId, tab] of tabs.entries()) {
    if (tab.state === 'connected') {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Mozi Browser Relay: relay reconnecting…',
      })
    }
  }
...
async function reannounceAttachedTabs() {
  for (const [tabId, tab] of tabs.entries()) {
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Mozi Browser Relay (click to attach/detach)',
      })
      continue
    }
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Mozi Browser Relay: attached (click to detach)',
      })
    } catch {
      setBadge(tabId, 'on')
    }
...
async function connectOrToggleForActiveTab() {
  ...
  try {
    ...
    void chrome.action.setTitle({
      tabId,
      title: 'Mozi Browser Relay: connecting to local relay…',
    })
    ...
    try {
      ...
      void chrome.action.setTitle({
        tabId,
        title: 'Mozi Browser Relay: relay not running (open options for setup)',
      })
...
function onDebuggerDetach(source, reason) {
  ...
  if (reason === 'target_closed') {
    ...
    void chrome.action.setTitle({
      tabId,
      title: 'Mozi Browser Relay: re-attaching after navigation…',
    })
