...
export async function createPtyAdapter(params: {
  ...
  // @ts-ignore - @lydell/node-pty has types in node-pty.d.ts but exports field doesn't point to them
  ...
  const onStderr = (_listener: (chunk: string) => void) => {
    // PTY gives a unified output stream; stderr is merged into stdout.
  };
