...
export async function searchVector(params: {
  ...
  if (await params.ensureVectorReady(params.queryVec.length)) {
    ...
    return rows.map((row) => ({
      ...
      score: 1 - row.dist,
      snippet: truncateText(row.text, params.snippetMaxChars),
      source: row.source,
    }));
  }
  ...
  return scored
    ...
    .map((entry) => ({
      ...
      score: entry.score,
      snippet: truncateText(entry.chunk.text, params.snippetMaxChars),
...
export async function mergeHybridResults(params: {
  vector: Array<SearchRowResult & { vectorScore: number }>;
  keyword: Array<SearchRowResult & { textScore: number }>;
  vectorWeight: number;
  textWeight: number;
}): Promise<SearchRowResult[]> {
  const byId = new Map<
