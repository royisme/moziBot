...
function makeAgentManager(messages: AgentMessage[], tapeServiceFn?: (sessionKey: string) => ReturnType<typeof createTapeService> | null) {
  return {
...
describe('Tape write in prompt-coordinator', () => {
  ...
  it('continues normally when tape write throws (non-fatal)', async () => {
    ...
    const brokenTapeService = {
      appendMessage: vi.fn(() => { throw new Error('disk full'); }),
      ...
      forkTape: vi.fn(),
      mergeFork: vi.fn(),
      info: vi.fn(() => ({ name: 'x', entries: 0, anchors: 0, lastAnchor: null, entriesSinceLastAnchor: 0 })),
      anchors: vi.fn(() => []),
