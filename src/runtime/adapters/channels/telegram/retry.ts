...
function isRecoverableError(error: unknown): boolean {
  ...
  // Telegram 5xx
  if (err.error_code && err.error_code >= 500) return true;
  // grammy HttpError with 5xx
  const msg = err.message || "";
  if (/5\d\d/.test(msg) && msg.includes("HTTP")) return true;
  return false;
}

function isRateLimitError(error: unknown): { isLimit: boolean; retryAfter: number } {
  if (!(error instanceof Error)) return { isLimit: false, retryAfter: 0 };
