/**
 * Shared utilities for bash-tools.
 * Ported from openclaw bash-tools.shared.ts
 */
...
/**
 * Slice a string safely handling UTF-16 boundaries.
 * This ensures we don't split surrogate pairs in the middle.
 */
...
/**
 * Coerce environment variables to string Record.
 * Filters out undefined and non-string values.
 */
...
async function assertSandboxPath(params: {
  ...
  if (path.isAbsolute(params.filePath)) {
    resolved = params.filePath;
  } else {
    resolved = path.resolve(params.cwd, params.filePath);
  }
  ...
  // If relative starts with '..', it's outside the sandbox
  if (relative.startsWith("..") && !path.isAbsolute(relative)) {
    throw new Error(`Path "${params.filePath}" is outside sandbox root "${root}"`);
  }
...
/**
 * Resolve sandbox workdir with fallback.
 */
...
/**
 * Split a string into chunks of specified limit.
 */
