...
export type ExtensionRelayStatus = { connected: boolean };
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Mozi-Extension-Relay",
          revision: "0",
          userAgent: "Mozi-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (path === "/extension/status") {
      ...
      res.end(JSON.stringify({ connected: extensionConnected() } satisfies ExtensionRelayStatus));
      return;
    }
...
export async function stopAllChromeExtensionRelays(): Promise<void> {
  const entries = Array.from(relayRuntimeByPort.values());
  await Promise.all(entries.map((entry) => entry.server.stop()));
  relayRuntimeByPort.clear();
}
