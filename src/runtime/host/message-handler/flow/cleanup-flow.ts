...
export const runCleanupFlow: CleanupFlow = async (ctx, deps, _bundle) => {
  ...
  const restoreModelRef = typeof state.capabilityRestoreModelRef === 'string' ? state.capabilityRestoreModelRef : undefined;
  const stopTyping = typeof state.stopTyping === 'function' ? state.stopTyping as (() => Promise<void> | void) : undefined;

  // Dependency Extraction
  const setSessionModel = requireFn<(sk: string, m: string) => Promise<void>>(deps, 'setSessionModel');
  const emitPhase = requireFn<(p: { phase: InteractionPhase; payload: PhasePayload }) => Promise<void>>(deps, 'emitPhaseSafely');
  const stopTypingIndicator = requireFn<(p: { stop?: () => Promise<void> | void; sessionKey: string; agentId: string; peerId: string }) => Promise<void>>(deps, 'stopTypingIndicator');
  const logger = requireObj<{ warn: (o: Record<string, unknown>, m: string) => void }>(deps, 'logger');
