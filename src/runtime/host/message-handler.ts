...
export class MessageHandler {
  ...
  private evaluateSemanticLifecycle(params: {
    ...
    if (!policy.enabled) {
      return { shouldRotate: false, shouldRevert: false, confidence: 0, threshold: policy.threshold };
    }

    let controlModelRef: string | undefined;
    try {
      controlModelRef = this.agentManager.resolveLifecycleControlModel({ sessionKey, agentId }).modelRef;
    } catch {
      controlModelRef = undefined;
    }

    const session = this.sessions.getOrCreate(sessionKey, agentId);
    const lifecycleMeta = (session.metadata?.lifecycle as Record<string, unknown> | undefined) || {};
    const semanticMeta =
